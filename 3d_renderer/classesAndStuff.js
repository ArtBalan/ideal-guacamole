...
class Item {
    ...
    translateItem(factor, tcenter) {
        ...
        if (!tcenter)
            this.center.translatePoint(factor);
    }
    /**
     ...
     *
     */
    scaleItem(factor, origin, scenter) {
        this.vectList.forEach((vectList) => {
            vectList.forEach((vect) => {
        ...
        if (!scenter)
            this.center.scalePoint(factor, origin);
    }
    /**
     ...
     */
    rotateItem(factor, origin, rcenter) {
        this.vectList.forEach((vectList) => {
            vectList.forEach((vect) => {
        ...
        if (!rcenter)
            this.center.rotatePoint(factor, origin);
    }
    centerised() {
        let sumX = 0;
        let sumY = 0;
...
class Circle extends Item {
    constructor(center, radius, segment, item) {
        ...
        for (let i = 0; i < segment; i++) {
            let tempPoint = new Vect(Math.cos(angle * i) * tempR + this.center.x, Math.sin(angle * i) * tempR + this.center.y, this.center.z);
